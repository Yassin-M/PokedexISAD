...
class EreduKontroladorea:
  ...
  def bilatu_eboluzioak(self, start_id, aurreko=True):
      ...
      while itzuli:
          ...
          bisitatuak.add(unekoa) # Bestela, bisitatuta zerrendara gehitu

          # SQL kontsulta: aurreko edo hurrengo eboluzioak lortzeko
          if aurreko:
              sql = """
  ...
  def getOnenak(self, talde_info):
    ...
    sql = """
          SELECT pt.PokemonPokedexID,
                 ...
                 pd.irudia
          FROM Taldea t
                   LEFT JOIN PokemonTaldean pta --Hautatutako taldeko pokemonak lortu
                             ON pta.taldeIzena = t.taldeIzena
                                 AND pta.erabiltzaileIzena = t.erabiltzaileIzena
                   LEFT JOIN PokemonTalde pt --Pokemon bakoitzaren informazioa lortu: hp, atk...
                             ON pt.harrapatuId = pta.harrapatuId
                                 AND pt.ErabiltzaileIzena = pta.erabiltzaileIzena
                   LEFT JOIN PokemonPokedex pd --Pokemon bakoitzaren irudia lortu
